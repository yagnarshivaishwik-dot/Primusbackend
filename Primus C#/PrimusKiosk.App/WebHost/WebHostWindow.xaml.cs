...
public partial class WebHostWindow : Window
{
    ...
    private async void OnLoaded(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            // https://primus.local/ and calls https://api.primustech.in/* via Axios.
            // Because these are different origins, the embedded Chromium would normally
            // block every XHR/fetch with a CORS preflight failure (Axios reports this
            // as "Network Error"). --disable-web-security removes that restriction for
            // the embedded browser only — it has zero effect on the C# HTTP pipeline
            // and is a standard practice for locked-down kiosk WebView2 deployments.
            ...
            // Serve the React build as https://primus.local/ — WebView2 treats this as a
            // first-party HTTPS origin so fetch / WebSocket / crypto APIs all work.
            ...
            core.Navigate("https://primus.local/index.html");
        }
        catch (Exception ex)
        {
