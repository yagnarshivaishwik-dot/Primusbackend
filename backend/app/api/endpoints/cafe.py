...
@router.post("/onboard")
async def register_cafe_public(data: CafeOnboard, db: Session = Depends(get_db)):
    if db.query(User).filter_by(email=data.email).first():
        raise HTTPException(status_code=400, detail="Email already registered")
    if db.query(Cafe).filter_by(name=data.cafe_name).first():
        raise HTTPException(status_code=400, detail="Cafe name already taken")

    cafe, owner, license_key = await onboard_cafe(db, data)
    return {
        "status": "success",
        "message": "Cafe onboarded successfully. Check your email for credentials.",
        "cafe": cafe.name,
    }
