...
def _get_cafe_db(ctx: AuthContext):
    """Open a cafe-scoped DB session. Raises 400 if cafe_id is unresolvable."""
    if not ctx.cafe_id:
    ...
    async def _compute():
        def _q():
            ...
            try:
                ...
                return {"series": series, "period": {"start": start_dt.isoformat(), "end": end_dt.isoformat()}}
            finally:
                db.close()
        return await run_in_threadpool(_q)
    ...
    async def _compute():
        def _q():
            ...
            try:
                ...
                return [
                    {"name": r.name, "price": float(r.price or 0), "qty": int(r.qty or 0), "revenue": float(r.revenue or 0)}
                    for r in rows
                ]
            finally:
                db.close()
    ...
    async def _compute():
        def _q():
            ...
            try:
                ...
                return [
                    {"hour": h, "label": f"{h:02d}:00", "sessions": hours.get(h, {}).get("sessions", 0), "revenue": hours.get(h, {}).get("revenue", 0.0)}
                    for h in range(24)
                ]
            finally:
                db.close()
        ...
        def _q():
            ...
            try:
                rows = (
                    ...
                    .filter(WalletTransaction.timestamp >= start_dt, WalletTransaction.timestamp < end_dt)
                    .group_by(WalletTransaction.type).all()
                )
                return [
                    {"method": r.method, "count": int(r.count or 0), "total": abs(float(r.total or 0))}
                    for r in rows
                ]
            finally:
                db.close()
...
async def analytics_superset_token(
    ...
    if not base:
        return {"enabled": False, "message": "Superset not configured. Set SUPERSET_BASE_URL in .env."}
    ...
    return {"enabled": True, "token": token, "embed_url": f"{base}/embedded/dashboard/{dashboard_id}"}
