...
# ============================================================================
# MULTI-DB CAFE-SCOPED MODEL OVERRIDE
# ============================================================================
# When MULTI_DB_ENABLED=true, the per-cafe tables (ClientPC, Booking, Order,
# RemoteCommand, SystemEvent, etc.) live in clutchhh_cafe_{cafe_id} databases
# whose schemas have NO cafe_id column (each database IS the cafe — the
# tenant boundary is at the connection level, not at the row level).
#
# The legacy classes defined above were designed for single-DB mode where
# every row carries cafe_id for tenant filtering. If we use them against a
# cafe-specific session in multi-DB mode, SQLAlchemy generates SQL like
# "WHERE cafe_id = ?" and crashes with UndefinedColumn.
#
# Rather than touching all 37+ endpoint files that do
#     from app.models import ClientPC, Booking, Order, ...
# we re-bind those specific names HERE to their cafe-scoped equivalents
# from app.db.models_cafe. After this block runs, "from app.models import
# Booking" returns the cafe-scoped class automatically when running in
# multi-DB mode.
#
# Globally-scoped models (User, Cafe, License, RefreshToken, UserCafeMap,
# PasswordResetToken, ClientUpdate, LicenseKey) are NOT overridden — those
# stay in the global database in both modes.
import os as _os

if _os.getenv("MULTI_DB_ENABLED", "false").lower() == "true":
    try:
        from app.db.models_cafe import (
            AuditLog as _CafeAuditLog,
            BackupEntry as _CafeBackupEntry,
            Booking as _CafeBooking,
            ChatMessage as _CafeChatMessage,
            ClientPC as _CafeClientPC,
            CoinTransaction as _CafeCoinTransaction,
            Coupon as _CafeCoupon,
            CouponRedemption as _CafeCouponRedemption,
            DeviceIpHistory as _CafeDeviceIpHistory,
            Event as _CafeEvent,
            EventProgress as _CafeEventProgress,
            Game as _CafeGame,
            HardwareStat as _CafeHardwareStat,
            Leaderboard as _CafeLeaderboard,
            LeaderboardEntry as _CafeLeaderboardEntry,
            LicenseAssignment as _CafeLicenseAssignment,
            MembershipPackage as _CafeMembershipPackage,
            Notification as _CafeNotification,
            Offer as _CafeOffer,
            Order as _CafeOrder,
            OrderItem as _CafeOrderItem,
            PC as _CafePC,
            PCGame as _CafePCGame,
            PCGroup as _CafePCGroup,
            PCToGroup as _CafePCToGroup,
            PlatformAccount as _CafePlatformAccount,
            PricingRule as _CafePricingRule,
            Prize as _CafePrize,
            PrizeRedemption as _CafePrizeRedemption,
            Product as _CafeProduct,
            ProductCategory as _CafeProductCategory,
            RemoteCommand as _CafeRemoteCommand,
            Screenshot as _CafeScreenshot,
            Session as _CafeSession,
            Setting as _CafeSetting,
            SupportTicket as _CafeSupportTicket,
            SystemEvent as _CafeSystemEvent,
            UserGroup as _CafeUserGroup,
            UserMembership as _CafeUserMembership,
            UserOffer as _CafeUserOffer,
            WalletTransaction as _CafeWalletTransaction,
            Webhook as _CafeWebhook,
        )

        # Re-bind names so `from app.models import X` picks up the cafe-scoped
        # version automatically. Order doesn't matter — these are direct rebinds.
        AuditLog = _CafeAuditLog
        BackupEntry = _CafeBackupEntry
        Booking = _CafeBooking
        ChatMessage = _CafeChatMessage
        ClientPC = _CafeClientPC
        CoinTransaction = _CafeCoinTransaction
        Coupon = _CafeCoupon
        CouponRedemption = _CafeCouponRedemption
        DeviceIpHistory = _CafeDeviceIpHistory
        Event = _CafeEvent
        EventProgress = _CafeEventProgress
        Game = _CafeGame
        HardwareStat = _CafeHardwareStat
        Leaderboard = _CafeLeaderboard
        LeaderboardEntry = _CafeLeaderboardEntry
        LicenseAssignment = _CafeLicenseAssignment
        MembershipPackage = _CafeMembershipPackage
        Notification = _CafeNotification
        Offer = _CafeOffer
        Order = _CafeOrder
        OrderItem = _CafeOrderItem
        PC = _CafePC
        PCGame = _CafePCGame
        PCGroup = _CafePCGroup
        PCToGroup = _CafePCToGroup
        PlatformAccount = _CafePlatformAccount
        PricingRule = _CafePricingRule
        Prize = _CafePrize
        PrizeRedemption = _CafePrizeRedemption
        Product = _CafeProduct
        ProductCategory = _CafeProductCategory
        RemoteCommand = _CafeRemoteCommand
        Screenshot = _CafeScreenshot
        Session = _CafeSession
        Setting = _CafeSetting
        SupportTicket = _CafeSupportTicket
        SystemEvent = _CafeSystemEvent
        UserGroup = _CafeUserGroup
        UserMembership = _CafeUserMembership
        UserOffer = _CafeUserOffer
        WalletTransaction = _CafeWalletTransaction
        Webhook = _CafeWebhook
    except ImportError:
        # If app.db.models_cafe is not available (e.g. partial install),
        # fall through to the legacy single-DB classes above. The endpoints
        # will still 500 in multi-DB mode but at least the app will boot.
        import logging as _logging
        _logging.getLogger(__name__).exception(
            "MULTI_DB_ENABLED=true but app.db.models_cafe could not be imported. "
            "Cafe-scoped endpoints will fall back to legacy classes and likely fail."
        )
